...
Runs [go2rtc](https://github.com/AlexxIT/go2rtc) for the TP-Link Unified HACS integration.

This add-on is the easy path for non-technical users: install, add camera IP/password, start. No Docker commands and no hand-written go2rtc YAML.

## Why this add-on exists

Many Tapo battery/solar cameras and doorbells do not expose normal RTSP on port 554. go2rtc supports TP-Link/Tapo's native `tapo://` source and provides:
...
For each camera, two streams are created:
